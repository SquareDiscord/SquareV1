...
class Coronavirus(commands.Cog):
    ...
    async def coronavirus(self, ctx, country=None, *, province=None):
        ...
        embed = discord.Embed(title="Coronavirus (COVID-19) stats", color=0x8A0606)
        embed.set_footer(
            text="These statistics are CONFIRMED and are due to change at anytime.")
    ...
    async def coronavirus_error(self, ctx, error):
        if isinstance(error, commands.CommandInvokeError):
            await ctx.send("<:TICKNOSQUARE:692171537498767380> **Invalid nation or state/providence.**")
    ...
    async def coronavirushistory(self, ctx, country="all", *, province=None):
        ...
        if province:
            data = await self.corona.get_province_history(country, province,
                                                          14)  # get the history for the given country with province
        else:
            data = await self.corona.get_country_history(country,
                                                         14)  # if no province given, get the data for whole country
        ...
        for i in range(14):  # get the previous 2 weeks
            case_history_value = "{}\n**{}:** \
                {}".format(case_history_value, data.case_history[i].date,
                           corona_api.format_number(data.case_history[i].value) if data.case_history[
                                                                                       i].value is not None else 'Unknown')
            death_history_value = "{}\n**{}:** \
                {}".format(death_history_value, data.death_history[i].date,
                           corona_api.format_number(data.death_history[i].value) if data.death_history[
                                                                                        i].value is not None else 'Unknown')
            recovery_history_value = "{}\n**{}:** \
                {}".format(recovery_history_value, data.recovery_history[i].date,
                           corona_api.format_number(data.recovery_history[i].value) if data.recovery_history[
                                                                                           i].value is not None else 'Unknown')
